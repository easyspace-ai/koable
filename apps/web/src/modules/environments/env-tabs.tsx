  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-xs text-muted-foreground">{t("knowledgeTab.fileCount", { count: knowledge.length })}</span>
        <button onClick={() => setAdding(!adding)} className="flex items-center gap-1 rounded px-2 py-1 text-[10px] text-muted-foreground hover:bg-muted hover:text-foreground">
          {adding ? <X className="h-3 w-3" /> : <Plus className="h-3 w-3" />} {adding ? t("shared.cancel") : t("knowledgeTab.addFile")}
        </button>
      </div>
      {adding && (
        <div className="rounded-md border bg-muted/20 p-2.5 space-y-2">
          <input type="text" value={filename} onChange={(e) => setFilename(e.target.value)} placeholder={t("knowledgeTab.filenamePlaceholder")} autoFocus
            className="w-full rounded border bg-background px-2 py-1.5 text-xs outline-none focus:ring-1 focus:ring-ring" />
          <textarea value={content} onChange={(e) => setContent(e.target.value)} placeholder={t("knowledgeTab.contentPlaceholder")} rows={6}
      ...
      {knowledge.length === 0 && !adding && (
        <div className="flex flex-col items-center py-8 text-center">
          ...
          <p className="text-xs text-muted-foreground">{t("knowledgeTab.emptyTitle")}</p>
          <p className="text-[11px] text-muted-foreground/60 mt-1">{t("knowledgeTab.emptyDescription")}</p>
        </div>
      )}
        ...
        {knowledge.map((k) => (
          <div key={k.id} className="rounded-md border hover:border-foreground/20 transition-colors">
            <div className="flex items-center gap-2 px-3 py-2 group">
              ...
              <span className="text-[10px] text-muted-foreground">{t("knowledgeTab.chars", { count: k.content.length })}</span>
              <button onClick={() => { setEditingFile(editingFile === k.filename ? null : k.filename); setEditContent(k.content); }}
                className="rounded p-0.5 text-muted-foreground opacity-0 group-hover:opacity-100 hover:text-foreground" title={t("knowledgeTab.editTitle")}>
                <Pencil className="h-3 w-3" />
              </button>
              <button onClick={() => void handleRemove(k.filename)}
                className="rounded p-0.5 text-muted-foreground opacity-0 group-hover:opacity-100 hover:text-destructive" title={t("knowledgeTab.removeTitle")}>
                <X className="h-3 w-3" />
              </button>
            </div>
            {editingFile === k.filename && (
              <div className="border-t px-3 py-2 space-y-2">
                <textarea value={editContent} onChange={(e) => setEditContent(e.target.value)} rows={8}
                  className="w-full rounded border bg-background px-2 py-1.5 text-xs font-mono outline-none focus:ring-1 focus:ring-ring resize-none" />
                <div className="flex justify-end gap-1">
                  <button onClick={() => setEditingFile(null)} className="rounded border px-2 py-1 text-xs hover:bg-muted">{t("shared.cancel")}</button>
                  <button onClick={() => void handleUpdate(k.filename)} disabled={savingEdit}
                    ...
                    {savingEdit && <Loader2 className="h-3 w-3 animate-spin" />} <Check className="h-3 w-3" /> {t("shared.save")}
                  </button>
                </div>
              </div>
            )}
...
export function SkillsTab({
  ...
  return (
    <div className="space-y-4">
      <RefPicker<ContextSkill> title={t("skillsTab.skillsTitle")} icon={<Sparkles className="h-3.5 w-3.5" />}
      ...
      <RefPicker<ContextRule> title={t("skillsTab.rulesTitle")} icon={<BookOpen className="h-3.5 w-3.5" />}
...
export function SettingsTab({
  ...
  return (
    <div className="space-y-3">
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground"><Star className="h-3.5 w-3.5" /> {t("settingsTab.activeEnvironment")}</div>
          <button onClick={() => setShowPicker(!showPicker)} className="flex items-center gap-1 rounded px-2 py-1 text-[10px] text-muted-foreground hover:bg-muted hover:text-foreground">
            {showPicker ? <X className="h-3 w-3" /> : <Pencil className="h-3 w-3" />} {showPicker ? t("shared.cancel") : t("settingsTab.change")}
          </button>
        </div>
        {!showPicker ? (
          <div className="rounded-lg border p-3">
            {projectEnvId ? (() => {
              ...
              ) : <p className="text-xs text-muted-foreground">{t("settingsTab.customLoading")}</p>;
            })() : (
              <div className="flex items-center gap-3">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-primary/20 text-lg">🌐</div>
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold">{t("settingsTab.workspaceDefault")}</span>
                    <Badge variant="default" className="text-[10px] px-1.5 py-0 bg-primary/20 text-primary border-primary/30">{t("settingsTab.badgeAuto")}</Badge>
                  </div>
                  <p className="text-xs text-muted-foreground">{t("settingsTab.inheritingDescription")}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="rounded-lg border bg-muted/10 p-2 space-y-1 max-h-64 overflow-y-auto">
            <button onClick={() => void handleAssign(null)} disabled={loadingAssign}
              ...
              <div className="flex-1 min-w-0"><span className="text-xs font-medium">{t("settingsTab.workspaceDefault")}</span><p className="text-[10px] text-muted-foreground">{t("settingsTab.inheritAllItems")}</p></div>
            ...
            {allEnvs.filter(e => e.scope !== 'project').length === 0 && (
              <p className="text-[11px] text-muted-foreground text-center py-3">{t("settingsTab.noCustomEnvironments")}</p>
            )}
          </div>
        )}
      </div>
      <div className="space-y-1">
        <div className="flex items-center gap-2 text-xs font-medium text-muted-foreground"><Boxes className="h-3.5 w-3.5" /> {t("settingsTab.workspaceDefaults")}</div>
